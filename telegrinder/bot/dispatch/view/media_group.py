...
class MediaGroupView(BaseView):
        ...
        async with self.lock:
            ...
            if len(group_data.messages) >= MAX_GROUP_PARTS:
                ...
                group_data.timer_task = asyncio.create_task(
                    self.wait_and_process(media_group_id, api, context)
                )
    ...
    async def process_media_group(self, initiating_event: Update, messages: List[MessageCute], api: API, context: Context) -> None:
        if not messages:
            return
