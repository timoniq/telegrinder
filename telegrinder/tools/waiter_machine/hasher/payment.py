...
def get_shipping_query_id_from_event(event: ShippingQuery) -> str:
    return event.id


def get_shipping_query_user_id_from_event(event: ShippingQuery) -> int:
    return event.from_user.id


def get_pre_checkout_query_id_from_event(event: PreCheckoutQuery) -> str:
    return event.id


def get_pre_checkout_query_option_from_event(event: PreCheckoutQuery) -> str:
    return event.invoice_payload


def get_pre_checkout_query_user_id_from_event(event: PreCheckoutQuery) -> int:
    return event.from_user.id


def get_pre_checkout_query_option_and_user_id_from_event(event: PreCheckoutQuery) -> tuple[str, int]:
    return event.invoice_payload, event.from_user.id


def get_paid_media_purchased_user_id_from_event(event: PaidMediaPurchased) -> int:
    return event.from_user.id
