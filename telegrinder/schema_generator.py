...
def generate(path: str, schema_url: str = URL) -> None:
            ...
            file.writelines([
                ...
                *([SPACES + "pass\n"] if not params else (
                    SPACES + "{}: {}\n".format(
                        ...
                        "typing.Optional[" + convert_type(param["type"]) + "] = None"
                    ) for param in params if param["name"] != "flags"
                ))
            ])
    ...
    with open(path + "/methods.py", "w") as file:
        file.writelines([
            ...
            SPACES + SPACES + "n = {k: v for k, v in loc.items() if k not in (\"self\", \"other\") and v is not None}\n",
    ...
    for method in methods:
        ...
        lines.extend([
            SPACES + li for li in (
                ...
                "if result.is_ok:\n",
                SPACES + "return Result(True, value=" + (
